    ...
    <div class="main-container">
      ...
      <div class="breadcrumb-container">
        <el-breadcrumb separator-class="el-icon-arrow-right">
          ...
          <el-breadcrumb-item>秸秆利用补助申请</el-breadcrumb-item>
        </el-breadcrumb>
      </div>
      ...
      <div class="form-container">
        ...
        <el-form :model="formData" :rules="rules" ref="form" label-width="150px" :disabled="!areaId.length || (status > 0 && is_draft == 0)" label-position="left">
          ...
          <div class="sub-title">秸秆综合利用补助资金申请</div>
          <el-row :gutter="20">
            <el-col :span="12">
              <el-form-item label="秸秆类型" prop="straw_type">
                <el-input maxlength="50" v-model.trim="formData.straw_type" placeholder="请输入秸秆类型"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="利用方式" prop="straw_way">
                <el-input maxlength="50" v-model.trim="formData.straw_way" placeholder="请输入利用方式"></el-input>
              </el-form-item>
            </el-col>
          </el-row>
          <el-row :gutter="20">
            <el-col :span="12">
              <el-form-item label="收集数量(吨)" prop="collect_num">
                <el-input-number
            ...
            <el-col :span="12">
              <el-form-item label="折干数量(吨)" prop="dry_num">
                <el-input-number
                  class="inputNumberSet"
                  controls-position="right"
                  :min="0"
          ...
          <el-row :gutter="20">
            <el-col :span="12">
              <el-form-item label="申请补助资金(万元)" prop="apply_straw_amount">
                <el-input-number
                  class="inputNumberSet"
                  controls-position="right"
                  :min="0"
      ...
      <div>
        <recordCon type="7" :basicTable="basicTable" v-if="status > 0" />
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        straw_type: [{ required: true, message: '请输入', trigger: 'blur' }],
        straw_way: [{ required: true, message: '请输入', trigger: 'blur' }],
        collect_num: [{ required: true, message: '请输入', trigger: 'blur' }],
        address: [{ required: true, message: '请输入', trigger: 'blur' }],
        apply_straw_amount: [{ required: true, message: '请输入', trigger: 'blur' }],
        dry_num: [{ required: true, message: '请输入', trigger: 'blur' }],
        mobile: [
  ...
  methods: {
    ...
    getData(project_id) {
      this.ApiLists.getJgSubsidy({ project_id }).then((res) => {
    ...
    updateImage(whichType, id) {
      this.ApiLists.getJgSubsidy({ project_id: this.$route.query.project_id }).then((res) => {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          if (imgsVlidate) {
            ...
            data.form_status = 1
            this.ApiLists.applyJgSubsidy(data)
              .then((res) => {
                if (res && res.code == 0) {
    ...
    finalData() {
      let data = {
        ...
        straw_type: this.formData.straw_type,
        straw_way: this.formData.straw_way,
        form_status: this.is_draft ? 0 : 1,
        collect_num: this.formData.collect_num,
        apply_straw_amount: this.formData.apply_straw_amount,
        mobile: this.formData.mobile,
        dry_num: this.formData.dry_num,
    ...
    onAddressChange(e) {
      ...
      this.ApiLists.getJgSubsidyId(params).then((res) => {
        if (res && res.code === 0) {
          this.$router.replace({
            path: '/straw/index',
            query: {
